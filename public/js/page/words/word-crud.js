            ...
            <tr>
                ...
                <td class="text-center align-middle w-5">
                    ...
                    <div class="table-links">
                        ...
                        <a href="#">Edit</a>
                        <div class="bullet"></div>
                        <a href="#" class="text-danger" id="btl-delete">Hapus</a>
                    </div>
                </td>
...
// Delete word row
$(document).on('click', '.table-links .text-danger', function (e) {
    e.preventDefault();
...
// Edit Word Row
$(document).on('click', '.table-links a:contains("Edit")', function (e) {
    e.preventDefault();

    const $tr = $(this).closest('tr');
    // const wordId = $tr.find('.words').attr('id');
    const wordText = $tr.find('.words').text().trim();
    const translation = $tr.find('.translation').text().trim();

    console.log(translation);
    

    // fill modal form
    $('#word_text').val(wordText);
    $('#word_translation').val(translation);

    $('#form-add-word').modal('show');
})