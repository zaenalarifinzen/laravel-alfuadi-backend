...
export function initWordCrud({
    ...
    // get initial active wordgroup id from the swiper event
    let activeWordGroupId = getSlider?.()?.getActiveWordGroupId?.() ?? null;

    document.addEventListener("wordgroup:changed", (event) => {
        activeWordGroupId = event.detail?.wordgroupId ?? null;
    });
    ...
    $("#btn-add-word").on("click", function () {
        ...
        // get active wordgroup from the swiper event
        activeWordGroupId ??= $(".swiper-slide-active .word-group").data(
            "wordgroup-id",
        );
