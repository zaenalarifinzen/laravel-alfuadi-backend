...
export function initWordsPage({
    ...
    const wordGroupsPrefix = "exercise_wordgroups_";
    ...
    function storeData() {
        const exerciseId = config.initialData.id;
        const exerciseContent = config.initialData.content;

        if (exerciseContent) {
            const newKey = wordGroupsPrefix + exerciseId;
            const oldKey = Object.keys(localStorage).find((k) =>
                k.startsWith(wordGroupsPrefix),
            );
            const oldObject = oldKey
                ? JSON.parse(localStorage.getItem(oldKey))
                : null;
            const modified = oldObject?.modified;

            if (newKey === oldKey && modified) {
                iziToast.info({
    ...
    function boot() {
        ...
        const cachedData = JSON.parse(cachedRaw);
        // const cachedExerciseId = cachedData?.id;
